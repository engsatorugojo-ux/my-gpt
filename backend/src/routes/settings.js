...
router.get("/", requireAuth, async (req, res) => {
  ...
  if (!rows.length) return res.json({ openai_api_key: "", openai_model: "gpt-5.4-mini" });
  ...
  try {
    ...
    if (exists.rows.length) {
      ...
      await pool.query(
        ...
        [req.userId, openai_api_key || "", openai_model || "gpt-5.4-mini"]
      );
    }
    res.json({ ok: true });
...
function getDefaultModels() {
  return ["gpt-5.4-mini", "gpt-4o", "gpt-4o-mini", "gpt-4-turbo", "gpt-4", "gpt-3.5-turbo", "o1", "o1-mini", "o3-mini"];
}
